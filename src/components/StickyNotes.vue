...
function onPointerMove(e: PointerEvent) {
  ...
  // 鼠标在窗口外松开时收不到 pointerup，靠 buttons 判断按键已释放，避免"没按键也跟着鼠标跑"
  if (e.buttons === 0) {
    onPointerUp()
    return
  }
  ...
  const area = document.querySelector('.notes-area') as HTMLElement | null
  if (!area) return
...
// 卸载时清理监听；先复位 dragging，避免把一次中途中止的拖动当成用户操作写回服务端
onBeforeUnmount(() => {
  dragging = false
  onPointerUp()
})
