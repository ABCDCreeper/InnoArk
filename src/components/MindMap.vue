...
import { NButton, NSpace, NModal, NInput, NIcon, NEmpty, NText, useDialog } from 'naive-ui'
...
const layout = computed(() => {
  ...
  const build = (node: MindNode, depth: number) => {
    if (placed.has(node.id)) return // 数据异常（成环）时停止递归，避免无限递归
    placed.add(node.id)
    maxDepth = Math.max(maxDepth, depth)
    const children = byParent.get(node.id) ?? []
    children.forEach((c) => build(c, depth + 1))
    // 只按已定位的子节点求中点；成环等原因导致子节点未定位时按叶子处理
    const ys = children.map((c) => pos.get(c.id)?.y).filter((y): y is number => y !== undefined)
    let y: number
    if (ys.length === 0) {
      y = leafCursor
      leafCursor += 1
  ...
  // 兜底：父节点缺失的孤儿节点也要参与布局，否则取不到坐标会让整个看板渲染失败
  props.nodes.filter((n) => !placed.has(n.id)).forEach((n) => build(n, 0))
...
function confirmRemove() {
  ...
  if (!id) return
  dialog.warning({ title: '删除导图分支', content: '该节点及全部子节点都会删除，确定继续？', positiveText: '删除', negativeText: '取消',
    onPositiveClick: () => { emit('remove', id); selectedId.value = null } })
}
...
function onPointerMove(e: PointerEvent) {
  if (!dragStart) return
  // 鼠标在窗口外松开时收不到 pointerup，靠 buttons 判断按键已释放，结束拖动
  if (e.buttons === 0) {
    onPointerUp()
    return
  }
