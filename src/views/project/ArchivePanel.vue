...
import {
  ...
  NTimeline, NTimelineItem, useMessage, useDialog, NTable, NIcon,
} from 'naive-ui'
...
/**
 * 导出 PDF：走浏览器打印（打印对话框里选"另存为 PDF"）。
 * 纯前端方案不引入新的服务端依赖，且打印样式会把界面控件一并隐去，
 * 输出的就是档案正文。
 */
function exportPdf() {
  window.print()
  ...
  <n-card size="small" :loading="loading">
    <template #header>
      <n-space align="center" justify="space-between">
        ...
        <n-button v-if="archive" size="small" quaternary class="no-print" @click="exportPdf">
      ...
      <n-card title="系统反馈" size="small" style="margin-bottom: 16px;">
        ...
        <n-timeline v-else>
          <n-timeline-item
            ...
            :key="f.id"
            :content="f.content"
            :time="formatTime(f.createdAt, true)"
      ...
      <n-card title="教师批注" size="small">
        ...
        <n-timeline v-else>
          <n-timeline-item
            ...
            :title="a.userName ?? '教师'"
            :content="a.content"
            :time="formatTime(a.createdAt, true)"
            type="warning"
          />
        </n-timeline>
      </n-card>
...
<style scoped>
@media print {
  /* 打印时只保留档案正文：隐藏应用外壳、页签与操作按钮 */
  :global(.n-layout-sider),
  :global(.n-layout-header),
  :global(.n-tabs-nav),
  :global(.n-card-header__extra),
  .no-print {
