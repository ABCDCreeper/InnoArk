...
import type { GroupInvite, GroupMember, QuizGroup, QuizMode, QuizQuestion, School, UserBrief } from '../api/types'
...
// 列表与侧栏各用一个守卫：load() 内部会调用 loadPanel()，共用一个计数器会让
// 先发起的那次因「已不是最新」而丢弃自己的结果。
const listTask = useAsyncTask(message, '分组加载失败')
const panelTask = useAsyncTask(message, '分组详情加载失败')
const loading = listTask.loading
const panelLoading = panelTask.loading
...
function load() {
  return listTask
    ...
    .then((ok) => {
      if (ok && selectedId.value) void loadPanel()
    })
}

// 快速切换分组时，先发出的请求可能后返回；守卫保证只有最新一次的结果被采纳
function loadPanel() {
  const id = selectedId.value
  if (!id) return Promise.resolve()
  return panelTask.run(
    () => Promise.all([fetchGroupMembers(id), fetchGroupQuestions(id), fetchGroupInvites(id)]),
    ([m, q, pi]) => {
...
const groupForm = ref({
  name: '', description: '', quizMode: 'group' as QuizMode,
  // 归属学校只允许平台管理员调整，校管理员不能把组划到自己名下
  schoolId: null as string | null,
})
// 学校列表仅平台管理员需要，按需加载一次
const schools = ref<School[]>([])
const canAssignSchool = computed(() => ROLE_RANK[auth.user?.role ?? 'student'] >= ROLE_RANK.admin)

async function loadSchools() {
  if (!canAssignSchool.value || schools.value.length) return
  try {
...
function removeMember(m: GroupMember) {
  ...
  dialog.warning({
    title: '移除成员',
    content: `确定将「${m.name}」移出分组？他/她在本组项目中的成员权限会被同时收回。`,
    positiveText: '移除',
    negativeText: '取消',
    onPositiveClick: async () => {
      try {
  ...
  <n-space vertical size="large">
    ...
    >
      <n-space vertical size="medium">
        ...
        <div v-if="canAssignSchool && schools.length">
          <n-text depth="3" style="font-size: 13px; display: block; margin-bottom: 6px;">归属学校</n-text>
          <n-select
            v-model:value="groupForm.schoolId"
            clearable
            placeholder="未归属（任何管理员可见）"
            :options="schools.map((sc) => ({ label: sc.name, value: sc.id }))"
          />
        </div>
        <n-space justify="end">
