...
const handleRegister = async () => {
  ...
  // 声明的 rules 必须显式触发才会生效，否则空表单会直接打到后端并回显原始校验错误
  try {
...
<template>
  <div class="register-container">
    <n-card title="注册 InnoArk" subtitle="智能跨学科学习平台" class="register-card" size="large">
      <n-form ref="formRef" :model="formValue" :rules="rules" label-placement="top" @submit.prevent="handleRegister">
        ...
        <n-text depth="3">学生注册；教师账号请联系管理员开通。</n-text>
        <n-button type="primary" attr-type="submit" size="large" block :loading="loading" :disabled="loading" style="margin-top: 8px;">
          注册
        </n-button>
      </n-form>
