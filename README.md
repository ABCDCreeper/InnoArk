...
| 成果归档 | 项目结题后自动生成完整科创档案（成员贡献、任务清单、打卡与反馈汇总），可一键生成结题证书并打印/存为 PDF |
| AI 助教小智 | 全局悬浮问答助手，数据感知：项目进度、错题分析、专注报告、排行榜名次、学习建议 |
| 排行榜 | 总榜/周榜双榜（积分 = 最佳闯关×0.5 + 专注分钟 + 打卡×2），前三名领奖台，高亮我的名次 |
| 学习周报 | 近 7 天专注/闯关/签到汇总与点评 |
| 快速跳转 | `Ctrl+K` 全局搜索页面 / 项目 / 资源库 |
| CSV 导出 | 任务清单、打卡记录、排行榜一键导出（带 BOM，Excel 直开不乱码） |
| 截止提醒 | 首页提醒卡片 + 看板逾期/临期彩色徽标（未来 7 天窗口） |
| 健康提醒 | 连续学习达到设定时长全局弹通知，间隔可调（含 1 分钟演示档），设置持久化 |
| 教师数据看板 | 团队任务四段分布条 + 成员专注/打卡聚合（教师端） |
| 签到热力图 | 近 12 周 GitHub 风格签到热力图（跟随当前账号） |
